...
pub(crate) fn init_tracing(log_level: Option<&str>) {
	INIT_TRACING.call_once(|| {
		...
		let (otel_layer, otel_error) = match rivetkit_core::telemetry::initialize_if_configured() {
			Ok(tracer) => (tracer.map(|tracer| {
...
pub async fn flush_telemetry() {
	rivetkit_core::telemetry::flush_best_effort(std::time::Duration::from_secs(5)).await;
}
